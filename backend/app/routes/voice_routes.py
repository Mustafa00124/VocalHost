...
def voice_entrypoint():
    ...
    if not assistant:
        resp = VoiceResponse()
        resp.say("Sorry, this assistant is not available.", voice="woman")
        resp.hangup()
        return Response(str(resp), mimetype="text/xml")
    
    # Check if assistant is accessible (owner has active subscription)
    if not assistant.is_accessible():
        resp = VoiceResponse()
        resp.say("This assistant is currently unavailable due to account status.", voice="woman")
        resp.hangup()
        return Response(str(resp), mimetype="text/xml")
